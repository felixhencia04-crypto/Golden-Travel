...
const targetStr = `      // 4. Batch Terdekat
      ...
      let nextBatchRegs = 0;
      let nextBatchName = nextBatch?.name || null;
      if (nextBatch) {
        const regs = await withRetry(() => db.select({ count: sql<number>\`count(*)\` })
          .from(schema.registrations)
          .where(eq(schema.registrations.packageId, nextBatch!.id)));
        nextBatchRegs = Number(regs[0]?.count || 0);
      }`;
...
const replacementStr = `      // 4. Batch Terdekat
      ...
      // Fallback to latest package if no schedule found
      if (!nextBatch) {
        const pkgCounts = await withRetry(() => db.select({
          packageId: schema.registrations.packageId,
          count: sql<number>\`count(*)\`
      ...
      let nextBatchRegs = 0;
      let nextBatchName = nextBatch?.name || (nextBatch as any)?.package?.name || null;
      if (nextBatch) {
        const regs = await withRetry(() => db.select({ count: sql<number>\`count(*)\` })
          .from(schema.registrations)
          .where(
            nextBatch.packageId === nextBatch.id 
            ? eq(schema.registrations.packageId, nextBatch.id)
            : eq(schema.registrations.scheduleId, nextBatch.id)
          ));
        nextBatchRegs = Number(regs[0]?.count || 0);
      }`;
...
if (content.includes(targetStr)) {
  ...
  console.log('Success Batch Terdekat');
} else {
  console.log('Target string Batch Terdekat not found');
}
